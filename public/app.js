...
const SPRINT_20 = [
    { id: 'T-052', name: 'Database Optimization', complexity: 'High', sp: 8, est: 40, act: 38, delay: 'No', team: 'Beta', exp: 'Senior', bugs: 1, status: 'Completed', risk: 'Low' },
    { id: 'T-053', name: 'Localization Setup', complexity: 'Medium', sp: 5, est: 25, act: 28, delay: 'Yes', team: 'Alpha', exp: 'Intermediate', bugs: 2, status: 'Completed', risk: 'Medium' },
    { id: 'T-054', name: 'User Avatar Upload', complexity: 'Low', sp: 3, est: 12, act: 10, delay: 'No', team: 'Gamma', exp: 'Junior', bugs: 0, status: 'Completed', risk: 'Low' },
    { id: 'T-055', name: 'Admin Dashboard', complexity: 'High', sp: 13, est: 60, act: 65, delay: 'Yes', team: 'Delta', exp: 'Senior', bugs: 4, status: 'Completed', risk: 'High' }
];

const SPRINT_19 = [
    { id: 'T-042', name: 'Initial Architecture setup', complexity: 'High', sp: 13, est: 70, act: 65, delay: 'No', team: 'Alpha', exp: 'Expert', bugs: 1, status: 'Completed', risk: 'Low' },
    { id: 'T-043', name: 'Core API Services', complexity: 'High', sp: 8, est: 45, act: 48, delay: 'Yes', team: 'Gamma', exp: 'Senior', bugs: 3, status: 'Completed', risk: 'Medium' },
    { id: 'T-044', name: 'Basic Styling Tokens', complexity: 'Low', sp: 2, est: 10, act: 9, delay: 'No', team: 'Beta', exp: 'Junior', bugs: 0, status: 'Completed', risk: 'Low' },
    { id: 'T-045', name: 'Authentication Mock', complexity: 'Medium', sp: 5, est: 20, act: 20, delay: 'No', team: 'Delta', exp: 'Intermediate', bugs: 1, status: 'Completed', risk: 'Low' }
];

// Sprint lookup map
const SPRINT_DB = {
    'sprint24.csv': { label: 'Sprint 24 (Current)', date: '04/07/26', data: SPRINT_24 },
    'sprint23.csv': { label: 'Sprint 23', date: '03/10/26', data: SPRINT_23 },
    'sprint22.csv': { label: 'Sprint 22', date: '02/10/26', data: SPRINT_22 },
    'sprint21.csv': { label: 'Sprint 21', date: '01/13/26', data: SPRINT_21 },
    'sprint20.csv': { label: 'Sprint 20', date: '12/15/25', data: SPRINT_20 },
    'sprint19.csv': { label: 'Sprint 19', date: '11/18/25', data: SPRINT_19 },
};
            ...
            <div class="team-item">
                <div class="team-info">
                    ...
                    <div>
                        ...
                        <div class="text-small">Lead: ${team.lead}</div>
                    </div>
                </div>
...
function initUploadScreen() {
    ...
    function parseCSV(csvText) {
        ...
        for (let i = 1; i < lines.length; i++) {
            ...
            const task = {};
            headers.forEach((header, index) => { task[header] = values[index]; });
            data.push({
                id: task.id || `T-${1000 + i}`,
                name: task.name || task.task || 'Unknown Task',
                complexity: task.complexity || 'Medium',
                sp: task.sp || task.storypoints || 0,
                est: task.est || task.estimated || 0,
                act: task.act || task.actual || 0,
                delay: task.delay || 'No',
                team: task.team || 'Unassigned',
                exp: task.exp || task.experience || 'Junior',
                bugs: task.bugs || 0,
    ...
    function processFile(file) {
        ...
        setTimeout(() => {
            ...
            if(upProg) upProg.style.width = '100%';
        }, 100);
        ...
        reader.onload = (e) => {
            ...
            setTimeout(() => {
                uploadZone.innerHTML = `
                    ...
                    <button class="btn btn-secondary" id="browse-files-btn" style="margin-top: 8px;">Browse Files</button>
                    <input type="file" id="csv-file-input" accept=".csv" style="display: none;">
                    <p class="text-small" style="margin-top: 8px;">Supported: .csv only | Max size: 10MB</p>
                `;
                // rebind events
                const newBrowse = document.getElementById('browse-files-btn');
                const newFileIn = document.getElementById('csv-file-input');
                if (newBrowse && newFileIn) {
                    newBrowse.addEventListener('click', (ev) => { ev.stopPropagation(); newFileIn.click(); });
                    newFileIn.addEventListener('change', (ev) => { if (ev.target.files && ev.target.files.length > 0) processFile(ev.target.files[0]); });
                }
                ...
                if (tbody) {
                    ...
                    newRow.innerHTML = `
                        <td><a href="#" class="sprint-file-link" data-sprint="${fileName}">${fileName}</a></td>
                        <td>${SPRINT_DB[fileName].date}</td><td>${parsedData.length}</td><td>✅</td>
                    `;
                    const newLink = newRow.querySelector('.sprint-file-link');
